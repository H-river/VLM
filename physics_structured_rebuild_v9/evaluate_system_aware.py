#!/usr/bin/env python3
"""Evaluate saved Qwen decisions with system-aware direction gates."""
...
DEFAULT_OUTPUT = (
    ...
    / "orchestrated_system_aware_direction_v9_validation.json"
)
...
def main() -> None:
    ...
    report["evaluation_version"] = (
        "saved_qwen_checkpoint1000_plus_system_aware_direction_v9"
    )
    report["scope"] = (
        "direction state/image routes use separately pinned system-aware v9 "
        "gates; forward remains shared v7; both inverse routes, measurement, "
        "and Qwen remain frozen"
    )
    report["artifacts"]["system_aware_state_direction_v9"] = {
        "path": str(DEFAULT_SYSTEM_AWARE_STATE_HYBRID.resolve()),
        "sha256": sha256(DEFAULT_SYSTEM_AWARE_STATE_HYBRID.resolve()),
    }
    report["artifacts"]["system_aware_image_direction_v9"] = {
        "path": str(DEFAULT_SYSTEM_AWARE_IMAGE_HYBRID.resolve()),
        "sha256": sha256(DEFAULT_SYSTEM_AWARE_IMAGE_HYBRID.resolve()),
    }
    output.write_text(
