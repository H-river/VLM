#!/usr/bin/env python3
"""Evaluate Qwen decisions with numerical forward/inverse v5 overlaid."""
...
from control_rebuild_v5.orchestrated_runtime import (
    OrchestratedNumericalRuntimeV5,
)
...
DEFAULT_V5_RUN = REPO_ROOT.parent / "VLM_runs/control_rebuild_v5_one_seed"
...
def main() -> None:
    ...
    output = (
        ...
        else v5_run / "orchestrated_system_numerical_v5_validation.json"
    )
    if output.is_file():
    ...
    torch, device = configure(20260728, args.device)
    v4_backend = OrchestratedSpecialistRuntimeV4.from_manifest(
        torch,
        args.v4_overlay.resolve(),
        device,
    )
    numerical_v5 = OrchestratedNumericalRuntimeV5(
        v4_backend,
        forward_artifact,
        inverse_artifact,
    )
    recording_runtime = RecordingRuntime(numerical_v5)
    ...
    if len(inverse_replay_results) != 2 * len(inverse_rows):
        raise RuntimeError("inverse replay recorder count differs from evaluation")
    ...
    metrics["physical_loss_versus_correctly_routed_specialist_by_task"] = losses
    metrics["physical_macro_loss_versus_correctly_routed_specialist"] = (
    ...
    report = {
        "evaluation_version": (
            "saved_qwen_checkpoint1000_plus_direction_v4_"
            "plus_numerical_forward_inverse_v5"
        ),
        "scope": (
            "forward state and image routes use forward v5; numerical "
            "state-inverse uses inverse v5; measurement, direction, and "
            "visual inverse remain frozen from the direction-v4 overlay"
        ),
        "device": str(device),
        "predictions": str(args.predictions.resolve()),
        "v4_overlay_manifest": str(args.v4_overlay.resolve()),
        "record_count": len(canonical),
        "task_ready_counts": task_ready_counts(canonical),
        "max_per_category": args.max_per_category,
        "numerical_v5_artifacts": {
            "forward": {
