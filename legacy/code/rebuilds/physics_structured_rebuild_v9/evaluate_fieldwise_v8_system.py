#!/usr/bin/env python3
"""Evaluate fieldwise direction plus v8 numerical inverse end to end."""
...
DEFAULT_OUTPUT = (
    ...
    / "orchestrated_fieldwise_direction_v8_inverse_validation.json"
)
...
def main() -> None:
    ...
    report["evaluation_version"] = (
        "saved_qwen_plus_fieldwise_direction_v9_plus_inverse_v8"
    )
    report["scope"] = (
        "direction uses route- and field-specific targeted-tree v9 gates; "
        "numerical-state inverse uses the frozen v8 Set Transformer with "
        "frozen v5 forward; visual inverse, forward, measurement, and Qwen "
        "remain frozen"
    )
    report["artifacts"]["fieldwise_state_direction_v9"] = {
        "path": str(DEFAULT_FIELDWISE_STATE_HYBRID.resolve()),
        "sha256": sha256(DEFAULT_FIELDWISE_STATE_HYBRID.resolve()),
    }
    report["artifacts"]["fieldwise_image_direction_v9"] = {
        "path": str(DEFAULT_FIELDWISE_IMAGE_HYBRID.resolve()),
        "sha256": sha256(DEFAULT_FIELDWISE_IMAGE_HYBRID.resolve()),
    }
