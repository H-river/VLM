#!/usr/bin/env python3
"""Evaluate the best accumulated v9 candidate through all 1,600 requests."""
...
DEFAULT_OUTPUT = (
    ...
    / "orchestrated_best_cycle_v9_validation.json"
)
...
def main() -> None:
    ...
    base_evaluator.OrchestratedSharedRuntimeV7 = OrchestratedBestCycleRuntimeV9
    sys.argv = [
        original_argv[0],
        "--output",
        str(output),
    ...
    report["evaluation_version"] = "saved_qwen_plus_best_v9_specialist_cycle"
    report["scope"] = (
        "forward uses route-specific residual-tree corrections over v7; "
        "direction uses route- and field-specific targeted-tree gates; "
        "numerical-state inverse uses the v8 Set Transformer; visual inverse, "
        "measurement, and Qwen remain frozen"
    )
    artifact_paths = {
