#!/usr/bin/env python3
"""Evaluate saved Qwen decisions with the shared forward-direction v6 overlay."""
...
from joint_forward_direction_v6.orchestrated_runtime import (
    OrchestratedSharedRuntimeV6,
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--v6-run", type=Path, default=DEFAULT_V6_RUN)
...
def main() -> None:
    ...
    v6_run = args.v6_run.resolve()
    shared_artifact = v6_run / "shared_forward_direction_v6.pt"
    forward_artifact = v5_run / "forward_tree_v5.pkl"
    inverse_artifact = v5_run / "inverse_tree_v5.pkl"
    output = (
        args.output.resolve()
        if args.output is not None
        else v6_run / "orchestrated_system_shared_v6_validation.json"
    )
    if output.is_file():
    ...
    torch, device = configure(20260729, args.device)
    v4_backend = OrchestratedSpecialistRuntimeV4.from_manifest(
        torch,
        args.v4_overlay.resolve(),
        device,
    )
    shared_v6 = OrchestratedSharedRuntimeV6(
        torch,
        v4_backend,
        shared_artifact,
        forward_artifact,
    ...
    recording_runtime = RecordingRuntime(shared_v6)
    ...
    report = {
        "evaluation_version": (
            "saved_qwen_checkpoint1000_plus_shared_forward_direction_v6"
        ),
        "scope": (
            "forward state/image and direction state/image routes use shared "
            "v6; numerical and visual inverse, measurement, and Qwen remain "
