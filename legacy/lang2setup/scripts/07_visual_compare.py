...
# ── project imports ──────────────────────────────────────────
VLM_ROOT = Path(__file__).resolve().parents[2]      # VLM/
...
def _load_base_config() -> dict:
    with open(VLM_ROOT / "optical_sim" / "configs" / "base_config.yaml") as f:
        return yaml.safe_load(f)


def _load_bins() -> dict:
    with open(VLM_ROOT / "lang2setup" / "configs" / "bins.yaml") as f:
...
def main():
    ...
    parser.add_argument("--predictions", type=str,
                        default=str(VLM_ROOT / "lang2setup" / "data" / "llm_predictions.jsonl"))
