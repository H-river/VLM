#!/usr/bin/env python3
"""Audit counterfactual-tool v9 privacy, reconstruction, and replayability."""
...
from .build_counterfactual_tool_v9 import STAGES, counterfactual_items, derive
...
def main() -> None:
    ...
    failures: list[str] = []
    reconstructed = materialized = replayed = 0
    for split, items in sources.items():
        ...
        if len(registry) != 2 * len(items):
            failures.append(f"{split}: paired state handles are not unique")
        for item in items:
            source = item["record"]
            rows = [row for row in actual if row["source_example_id"] == source["example_id"]]
            mapping = next(row for row in rows if row["stage"] == "tool_source_mapping")
            interpretation = next(
                row for row in rows if row["stage"] == "compact_tool_result_interpretation"
            )
            if not value_equal(
                materialize_counterfactual_answer(interpretation["prompt_inputs"]["tool_result"]),
                source["target"],
            ):
            ...
            if not value_equal(result, source["target"]["answer"]):
                failures.append(f"{split}: simulator replay mismatch {source['example_id']}")
            else:
                replayed += 1
    ...
    report = {
        ...
        "simulator_replay_count": replayed,
